        ...
        <div class="card m-2 border-0 shadow-sm">
          <div class="card-body">
            <div class="row row-cols-1 row-cols-sm-2">
              ...
              <p class="col font-weight-bold"><?= strtoupper($formvalue['id_paket']) ?></p>
            </div>
            <div class="row row-cols-1 row-cols-sm-2">
              <span class="col">Nama paket</span>
              <p class="col font-weight-bold"><?= $formvalue['nama_paket'] ?></p>
            </div>
            <div class="row row-cols-1 row-cols-sm-2">
              <span class="col">Harga</span>
              <p class="col font-weight-bold harga"><?= $formvalue['harga'] ?></p>
            </div>
            <div class="m-1">
              <a href="<?= base_url('datapaketsewa/details/pageedit/'. $formvalue['id_paket'] ) ?>" class="btn btn-primary">
                <svg class="bi mr-2" width="18" height="18" fill="currentColor">
                  <use href="<?= base_url('assets/img/bootstrap-icons-1.2.1/bootstrap-icons.svg#pencil-fill'); ?>" />
                </svg>
                Edit
              </a>
      ...
      <div class="col p-0">
        <div class="card m-2 border-0 shadow-sm">
          <ul class="list-group list-group-flush">
            ...
            <?php foreach ($formvalue['id_barang'] as $key => $value) : ?>
              <li class="list-group-item">
                <img src="<?= base_url('assets/resources/img/databarang/'. $formvalue['foto_barang'][$key]['foto_barang']) ?>" class="rounded-sm mr-2" alt="foto barang" onerror="this.onerror = null; this.src = '<?= base_url('assets/resources/img/noimg.png') ?>'" style="width:3em; height: 3em; object-fit: cover; ">
                <?= $formvalue['jumlah_barang'][$key] . ' ' . $formvalue['nama_barang'][$key] ?>
              </li>
            <?php endforeach ?>
          </ul>
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow-lg">
      ...
      <div class="modal-body">
        <p>Anda yakin ingin menghapus "<?= $formvalue['nama_paket'] ?>" dari daftar paket sewa?</p>
      </div>
      <div class="modal-footer bg-transparent border-0">
        <a data-dismiss="modal" class="btn text-danger">Batal</a>
        <a href="<?= base_url('datapaketsewa/hapus/'. $formvalue['id_paket']) ?>" class="btn btn-danger selected-data">Hapus</a>
      </div>
    </div>
  </div>
</div>
