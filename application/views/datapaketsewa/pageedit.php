<section class="col-md mt-3">
  ...
  <main class="mt-3 mx-2 mx-sm-5">
    <form action="<?= base_url('datapaketsewa/edit') ?>" method="post" novalidate>
      <div class="row">
        <div class="col-sm">
          <input type="hidden" name="paket[id_paket]" id="id_paket" value="<?= $formvalue['id_paket'] ?>">
          <div class="form-group">
            <label for="nama_barang">Nama Paket Sewa</label>
            <input type="text" required class="form-control" name="paket[nama_paket]" id="nama_paket" value="<?= $formvalue['nama_paket'] ?>">
            <div class="invalid-feedback">Harap tambahkan setidaknya 1 isi paket</div>
          </div>
          <div class="form-group">
            <label for="harga">Harga (Rp)</label>
            <input type="number" required class="form-control" name="paket[harga]" id="harga" value="<?= $formvalue['harga'] ?>" placeholder="Misal: 10000">
          ...
          <div class="form-group list-barang">
            ...
            <?php foreach ($formvalue['id_barang'] as $key => $value) : ?>
              <div class="input-group list-item">
                <input type="number" readonly name="paket[jumlah_barang][]" class="form-control bg-white jumlah" value="<?= $formvalue['jumlah_barang'][$key] ?>">
                <input type="hidden" name="paket[id_barang][]" class="form-control bg-white jumlah" value="<?= $formvalue['id_barang'][$key] ?>">
                <input type="text" readonly name="paket[nama_barang][]" class="form-control bg-white w-50 nama" value="<?= $formvalue['nama_barang'][$key] ?>">
