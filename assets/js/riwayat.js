...
<div class="row">
    ...
    <p class="text-muted my-0 tgl">${formatDate(riwayat.tgl_mulai, dt.DATE_MED)} ${formatTime(riwayat.waktu_mulai)} - ${formatDate(riwayat.tgl_selesai, dt.DATE_MED)} ${formatTime(riwayat.waktu_selesai)}</p>
    ...
    function toggleListEmpty(toggle, iconName = "", messageTitle = "", messageDetail = "") {
        switch (toggle) {
            case "show":
                ...
                listRiwayatEmptyMessage.children[0].querySelector("use").setAttribute("href", `${BASEURL}/assets/img/bootstrap-icons-1.2.1/bootstrap-icons.svg#${iconName}`);
